...
func TestVersionsFromReleaseBodyRegex(t *testing.T) {
	mainnetRegex := clientRepoConfigs[constants.ClientNameAgave].ReleaseNotesRegexes[constants.ClusterNameMainnetBeta]
	...
	tests := []struct {
		...
		{
			name: "matching releases with old mainnet beta phrase",
			releases: []*github.RepositoryRelease{
		...
		{
			name: "matching releases with new stable mainnet phrase",
			releases: []*github.RepositoryRelease{
		...
		{
			name: "matching releases with both mainnet phrases mixed",
			releases: []*github.RepositoryRelease{
				{Body: github.String("This is a stable release suitable for use on Mainnet Beta"), TagName: github.String("v3.1.10")},
				{Body: github.String("This is a stable Mainnet release"), TagName: github.String("v3.1.11")},
		...
		{
			name: "matching releases with v4 stable mainnet and testnet phrase",
			releases: []*github.RepositoryRelease{
				{Body: github.String("This a stable release suitable for Testnet, Devnet and Mainnet Beta."), TagName: github.String("v4.0.0")},
				{Body: github.String("This is Mainnet-beta Upgrade Candidate release. It is also recommended for Testnet and Devnet."), TagName: github.String("v4.0.0-rc.1")},
				{Body: github.String("Mainnet Upgrade Candidate. It is also recommended for Testnet and Devnet."), TagName: github.String("v4.1.0-rc.1")},
			},
			regex: mainnetRegex,
			want:  []string{"v4.0.0", "v4.0.0-rc.1", "v4.1.0-rc.1"},
		},
		{
...
func TestVersionsFromReleaseBodyRegexWithPrereleaseIncludesAgaveMainnetUpgradeCandidates(t *testing.T) {
	...
	releases := []*github.RepositoryRelease{
		...
		},
		{
			...
			TagName:    github.String("v4.1.0-beta.1"),
			Prerelease: github.Bool(true),
		},
		{
			Body:    github.String("This is a stable release suitable for use on Mainnet Beta."),
			TagName: github.String("v4.0.2"),
		},
	}

	got := versionsFromReleaseBodyRegexWithPrerelease(releases, mainnetRegex, true)
	assertVersionStringsEqual(t, got, []string{"v4.1.0-rc.1", "v4.0.2"})
}
...
func TestAgaveReleaseNotesRegexes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			regex: mainnetRegex,
			want:  []string{"v3.1.14", "v4.0.0-rc.0", "v4.1.0-rc.1", "v4.0.0", "v4.0.1"},
		},
		{
			name:  "testnet includes direct and recommended testnet releases",
			regex: testnetRegex,
			want:  []string{"v4.0.0-beta.7", "v4.0.0-beta.5", "v4.0.0-rc.0", "v4.1.0-rc.1", "v4.0.0", "v4.1.0"},
		},
	}
	...
	for _, cluster := range []string{constants.ClusterNameMainnetBeta, constants.ClusterNameTestnet} {
		t.Run(cluster, func(t *testing.T) {
			...
			got, err := client.latestVersionFromClusterVersionStrings(map[string][]string{
				constants.ClusterNameMainnetBeta: {"v3.1.14", "v4.0.0-rc.0", "v4.0.0-rc.1", "v4.0.0"},
				constants.ClusterNameTestnet:     {"v4.0.0-beta.7", "v4.0.0-rc.0", "v4.0.0-rc.1", "v4.0.0"},
			})
			if err != nil {
				t.Fatalf("latestVersionFromClusterVersionStrings() error = %v", err)
			}

			want, err := version.NewVersion("v4.0.0")
			if err != nil {
...
func TestJitoVersionStringsFromAgaveReleaseBodyRegex(t *testing.T) {
	...
	jitoReleases := []*github.RepositoryRelease{
		...
		{
			Name:       github.String("Testnet - v4.1.0-rc.1-jito"),
			TagName:    github.String("v4.1.0-rc.1-jito"),
			Prerelease: github.Bool(true),
		},
		{
